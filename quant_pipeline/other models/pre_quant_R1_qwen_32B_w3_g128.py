...
def run_awq(
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="Running QuantLRM..."):
        ...
        for matrix_name in ['gate', 'down']:
            minus = torch.load("QuantLRM/weight_difference/" + matrix_name + "/" + str(i) + ".pt").to(dtype=torch.float16)
            quadratic = torch.load("QuantLRM/quadratic/" + matrix_name + "/" + str(i) + ".pt").to(dtype=torch.float16)
            minus_chunks, B_parts = torch.tensor_split(minus, 5, dim=0), []
            for Bi in minus_chunks:
                bi_zero_count = (Bi == 0).sum(dim=0) + 1
                B_parts.append(bi_zero_count)
            count = (torch.stack(B_parts, dim=0).sum(dim=0) / 5).half()
